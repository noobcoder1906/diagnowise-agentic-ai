...
cred = credentials.Certificate(r"D:\DiagnoWise\service.json")
...
async def analyze_symptoms(data: SymptomRequest, user_data: dict = Depends(verify_firebase_token)):
    try:
...
@app.post("/report")
async def report(history: ReportRequest, user_data: dict = Depends(verify_firebase_token)):
    try:
...
@app.post("/report-pdf")
async def report_pdf(history: ReportRequest, user_data: dict = Depends(verify_firebase_token)):
    try:
        # Generate the medical report using CrewAI
        agent = medical_history_agent
        history_task = create_history_analysis_task(history.history, medical_history_agent)
...
async def download_pdf(request: dict, user_data: dict = Depends(verify_firebase_token)):
    try:
...
@app.post("/emergency")
async def emergency(message: EmergencyRequest, user_data: dict = Depends(verify_firebase_token)):
